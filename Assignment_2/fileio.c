...
// is made to modify a file such that "\x7F""ELF" appears in the first
// two bytes of the file, the write operation fails and ILLEGAL_ELF is
...
unsigned long write_file_at(File file, void *data, unsigned long num_bytes, 
  ...
  else if (offset == 0L && ! memcmp(data, "\x7F""ELF", 4)) {    
    // don't let "\x7F""ELF" ever appear at the beginning of the file!
